...
@Data
@ApiModel("租户销售订单详情")
public class MallSaleOrderDetailDTO {
    ...
    public MallSaleOrderDetailDTO afterPropertiesSet() {
        this.vitalNodes();
        stateDescAndTip();
    ...
    private void stateDescAndTip() {
        ...
        switch (orderStateEnum) {
            ...
            case CANCELED:
                this.stateDesc = "已取消";
                this.tip = this.cancelFrom == OperateFromEnum.SYSTEM ? "支付超时，订单自动取消" : this.cancelFrom == OperateFromEnum.BUYER ?
                        "客户取消订单，不想要了" : "商家取消订单，没货了";
                break;
            case REFUNDED:
                this.stateDesc = "已退款";
                this.tip = "所有商品已退款";
