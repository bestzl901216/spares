...
import lombok.Data;
import org.geely.domain.core.data.ProductData;
...
public class ProductCreateDTO {
    ...
    private List<ProductPropsCreateDTO> props;
    @ApiModelProperty(value = "商品详情图片")
    private List<String> images;

    public ProductData toData() {
        ProductData data = new ProductData();
        data.setMallId(getMallId());
        data.setSupplierId(getSupplierId());
        data.setShopId(getShopId());
        data.setBrandId(getBrandId());
        data.setQualityId(getQualityId());
        data.setName(getName());
        data.setDescription(getDescription());
        data.setCategoryId(getCategoryId());
        return data;
    }
